...
pub trait RungeKutta4thOrder2nd {
    fn cal_ddy(t: f64, v: &Vector3, x: &Vector3) -> Vector3;
    fn cal_dy(t: f64, v: &Vector3, x: &Vector3) -> Vector3;
    ...
    fn cal_slope(&self, dt: f64, (t, xs): (f64, &[&Vector3])) -> (Vector3, Vector3) {
        let (v, x) = (xs[0], xs[1]);
...
mod tests {
    ...
    fn test_rk4th2order() {
        ...
        for _ in 0..4 {
            t += dt;
            x.solve(t);
            assert!((x.velocity.z - (-GRAVITY * t)).abs() < error);
            assert!((x.position.z - (1. / 2. * (-GRAVITY) * t * t)).abs() < error);
        }
    }
